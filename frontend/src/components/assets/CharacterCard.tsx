...
export default function CharacterCard({ character, onUpload, onRemove, onDelete, generating }: CharacterCardProps) {
    ...
    <div className="group flex flex-col">
          ...
          <>
            ...
            {!uploading && !removing && (
              <div className="absolute bottom-2 right-2 flex gap-1.5 opacity-0 transition-all group-hover:opacity-100">
                ...
                {onRemove && (
                  <button
                    onClick={async () => { setRemoving(true); try { await onRemove(character.id); } finally { setRemoving(false); } }}
      ...
      <div className="mt-3 flex items-start justify-between">
        <div className="min-w-0">
          <h3 className="text-sm font-bold text-stone-900">{character.name}</h3>
          <p className="text-xs text-stone-500">
            Age {character.age} &bull; {character.role}
          </p>
        </div>
        {onDelete && (
          <button
            onClick={async () => { setDeleting(true); try { await onDelete(character.id); } catch { setDeleting(false); } }}
            disabled={deleting}
